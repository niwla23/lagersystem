...
// QueryBoxes chains the current query on the "boxes" edge.
func (sq *SystemQuery) QueryBoxes() *BoxQuery {
...
// WithBoxes tells the query-builder to eager-load the nodes that are connected to
// the "boxes" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *SystemQuery) WithBoxes(opts ...func(*BoxQuery)) *SystemQuery {
	query := (&BoxClient{config: sq.config}).Query()
...
func (sq *SystemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*System, error) {
	...
	if query := sq.withBoxes; query != nil {
		if err := sq.loadBoxes(ctx, query, nodes,
			func(n *System) { n.Edges.Boxes = []*Box{} },
			func(n *System, e *Box) { n.Edges.Boxes = append(n.Edges.Boxes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SystemQuery) loadBoxes(ctx context.Context, query *BoxQuery, nodes []*System, init func(*System), assign func(*System, *Box)) error {
	...
	query.withFKs = true
	query.Where(predicate.Box(func(s *sql.Selector) {
		s.Where(sql.InValues(system.BoxesColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.system_boxes
		if fk == nil {
			return fmt.Errorf(`foreign-key "system_boxes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "system_boxes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
