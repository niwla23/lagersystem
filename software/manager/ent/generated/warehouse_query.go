...
func (wq *WarehouseQuery) QueryPositions() *PositionQuery {
	query := (&PositionClient{config: wq.config}).Query()
...
func (wq *WarehouseQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (wq *WarehouseQuery) FirstIDX(ctx context.Context) int {
...
func (wq *WarehouseQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (wq *WarehouseQuery) OnlyIDX(ctx context.Context) int {
...
func (wq *WarehouseQuery) IDs(ctx context.Context) ([]int, error) {
	var ids []int
...
func (wq *WarehouseQuery) IDsX(ctx context.Context) []int {
	ids, err := wq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (wq *WarehouseQuery) Clone() *WarehouseQuery {
	...
	return &WarehouseQuery{
		...
		withPositions: wq.withPositions.Clone(),
...
func (wq *WarehouseQuery) WithPositions(opts ...func(*PositionQuery)) *WarehouseQuery {
	query := (&PositionClient{config: wq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	wq.withPositions = query
	return wq
}
...
func (wq *WarehouseQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Warehouse, error) {
	...
	if query := wq.withPositions; query != nil {
		if err := wq.loadPositions(ctx, query, nodes,
			func(n *Warehouse) { n.Edges.Positions = []*Position{} },
			func(n *Warehouse, e *Position) { n.Edges.Positions = append(n.Edges.Positions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (wq *WarehouseQuery) loadPositions(ctx context.Context, query *PositionQuery, nodes []*Warehouse, init func(*Warehouse), assign func(*Warehouse, *Position)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Warehouse)
	for i := range nodes {
	...
	query.Where(predicate.Position(func(s *sql.Selector) {
		s.Where(sql.InValues(warehouse.PositionsColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.warehouse_positions
		if fk == nil {
			return fmt.Errorf(`foreign-key "warehouse_positions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "warehouse_positions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
