...
func (pq *PropertyQuery) QueryPart() *PartQuery {
	query := (&PartClient{config: pq.config}).Query()
...
func (pq *PropertyQuery) WithPart(opts ...func(*PartQuery)) *PropertyQuery {
	query := (&PartClient{config: pq.config}).Query()
...
func (pq *PropertyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Property, error) {
	...
	if query := pq.withPart; query != nil {
		if err := pq.loadPart(ctx, query, nodes, nil,
			func(n *Property, e *Part) { n.Edges.Part = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PropertyQuery) loadPart(ctx context.Context, query *PartQuery, nodes []*Property, init func(*Property), assign func(*Property, *Part)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Property)
	for i := range nodes {
		if nodes[i].part_properties == nil {
			continue
		}
		fk := *nodes[i].part_properties
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "part_properties" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
