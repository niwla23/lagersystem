...
func (tq *TagQuery) QueryParts() *PartQuery {
	query := (&PartClient{config: tq.config}).Query()
...
func (tq *TagQuery) WithParts(opts ...func(*PartQuery)) *TagQuery {
	query := (&PartClient{config: tq.config}).Query()
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withParts; query != nil {
		if err := tq.loadParts(ctx, query, nodes,
			func(n *Tag) { n.Edges.Parts = []*Part{} },
			func(n *Tag, e *Part) { n.Edges.Parts = append(n.Edges.Parts, e) }); err != nil {
			return nil, err
		}
	}
...
func (tq *TagQuery) loadParts(ctx context.Context, query *PartQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Part)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.PartsTable)
		s.Join(joinT).On(s.C(part.FieldID), joinT.C(tag.PartsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.PartsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.PartsPrimaryKey[1]))
...
func (tq *TagQuery) loadChildren(ctx context.Context, query *TagQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Tag)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "tag_children" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
